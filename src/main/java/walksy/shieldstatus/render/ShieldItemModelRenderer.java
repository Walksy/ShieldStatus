...
public class ShieldItemModelRenderer {
    ...
    public void render(@Nullable ComponentMap componentMap, ShieldEntityModel model, ItemDisplayContext modelTransformationMode, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i, int j, boolean bl) {
        ...
        int colorWithAlpha = ColorHelper.withAlpha(color.getAlpha(), color.getRGB());
    ...
    private void renderLayer(MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay, ModelPart canvas, SpriteIdentifier textureId, DyeColor color, int statusColor) {
        canvas.render(matrices, textureId.getVertexConsumer(vertexConsumers, RenderLayer::getEntityTranslucent), light, overlay, ColorHelper.withAlpha(ColorHelper.getAlpha(statusColor), color.getEntityColor()));
    }
